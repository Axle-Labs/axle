...
Once a service has expressed interest in leveraging Axle, use the [`POST /platform/client`](/api-reference/platform/create-client) endpoint to register it as a new destination client.
...
If you do happen to lose the destination client `id`, you can retrieve it using the [`/platform/clients`](/api-reference/platform/create-client) endpoint to get a list of all destination clients registered with your platform.
