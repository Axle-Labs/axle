...
All events include a `client` parameter with the destination client `id`, so you can easily associate an event with the correct destination client. Please refer to our guides on [Ignition events](/guides/ignition-events) and [Account](/guides/account-events) or [Policy](/guides/policy-events) events for more details.
