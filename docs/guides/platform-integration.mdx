...
Our platform partners are typically software providers that provide many services to their customers, and want to offer Axle as a feature of their platform.
For example, a dealership management system might be a platform with many dealerships as customers, and these dealerships want to use Axle to verify insurance information for their own users, but may not want to setup Axle themselves.
As they're already acccustomed to using the certain dealership management system, it will be easier for them to use Axle through that system.
