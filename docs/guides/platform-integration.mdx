...
These dealerships want to use Axle to verify insurance information for their own customers, but my not want to setup Axle themselves.
...
Platforms can configure a new destination client using the [Create Destination Client API](/clients/create-destination-client).
This will return a unique `destination client id`, which the platform should store in their database alongside their internal unique identifier for the destination client,
...
Store the `destinationClientId` in your database alongside your internal unique identifier for the destination client. This will be used to authorize requests to Axle's APIs on behalf of the destination client.
