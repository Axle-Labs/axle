...
<Tip>
  ...
  email or Slack by making a request to the same Axle API routes. Reach out to
  the Axle team to ensure your `client` is properly configured!
</Tip>

The following [Account events](/api-reference/sandbox/trigger-account-event) and [Policy events](/api-reference/sandbox/trigger-policy-event) are supported:

**account.modified**

You can modify Account attributes to simulate `account.modified` event. The following example will change `email`, `firstName` and `phone` attributes on an existing account object.
...
<Info>
     When account has been set as `disconnected`, user needs to complete new Axle session. If the user reconnects the same insurance account and policy, Axle will
      merge the updated information with any existing Account or Policy objects. More info on `account.disconnected` can be found [here](/guides/account-events#account-disconnected).
</Info>

**policy.modified**

You can modify Account attributes to simulate `policy.modified` event. The following example will set Policy as inactive (e.g. expirationDate in the past) and update an address. 
