...
A `POST` request will be sent to the `webhookUri` with the following payload. All events will include `client`, `ref` (Policy object identifier), `user` (optionally specified when generating Ignition token), and `metadata` (optionally specified when generating Ignition token).
...
When identifying details on the Policy change. Refer to [Policy](/api-reference/policies/policy) for more details on the attributes that can be modified.
