...
```
  ...
  <Accordion title="Address Match">
    ...
    | Status    | Cause                                                                |
    | --------- | -------------------------------------------------------------------- |
    | `pass`    | Input `addressLine1`, `addressLine2`, and `postalCode` matches Axle policy  |
    | `caution`    | Input `addressLine1` and `postalCode` match, but `addressLine2` does not match  |
    | `fail`    | Either `addressLine1` or `postalCode` do not match between provided input and Axle policy address    |
    | `unknown` | Axle policy does not have addressLine1 or postalCode. Alternatively, it could indicate an error during validation.                              |

    **Input Parameters**
    - `address`: Full address provided as a `string` that includes both Address Line 1 and 2 along with postal code, and state.
