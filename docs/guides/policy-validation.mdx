...
<Accordion title="Third Party Name and Address Match">

### `third-party-name-address-match`

Evaluates whether the third party (e.g., lender) listed on the policy matches the provided name and, if provided, the address. The rule checks for a substring or fuzzy match on the name and validates the address using an existing address match rule.

#### Input Parameters

- `name` (required): String representing the name of the third party (e.g., "Sunrise Bank").
- `address` (optional): String or object representing the address of the third party.

#### Responses

| Status    | Cause                                                                                 |
| --------- | ------------------------------------------------------------------------------------- |
| `pass`    | Name is a substring or high-confidence fuzzy match AND address matches (if provided). |
| `caution` | Name is a medium-confidence fuzzy match OR address match results in caution.          |
| `fail`    | Name does not match OR address match fails.                                           |
| `unknown` | Name is null OR address match results in unknown.                                     |

#### Example Usage

````typescript
{
  "rule": "third-party-name-address-match",
  "input": {
    "name": "Sunrise Bank",
