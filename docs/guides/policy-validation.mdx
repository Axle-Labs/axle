...
```
  ...
  <Accordion title="Bodily Injury Coverage Meets Requirements">
    ...
    **Evaluation Criteria**
    ...
    2. `bodily-injury-limit-per-person-comparison` (if applicable): Determines if the `limitPerPerson` specified on the bodily injury coverage on the policy is greater than or equal to the provided `limitPerPerson`.

    3. `bodily-injury-limit-per-accident-comparison` (if applicable): Determines if the `limitPerAccident` specified on the bodily injury coverage on the policy is greater than or equal to the provided `limitPerAccident`.
  ...
  <Accordion title="Property Damage Coverage Meets Requirements">
    ...
    **Evaluation Criteria**
    ...
    The Rule is evaluated based on the following criteria, depending on if any limit was passed in. If no limits are passed in, then the Rule is evaluated based off if property damage coverage exists on the policy.

    1. `property-damage-exists`: Determines if property damage coverage is present on the policy.

    2. `property-damage-limit-per-accident-comparison` (if applicable): Determines if the `limitPerAccident` specified on the property damage coverage on the policy is greater than or equal to the provided `limitPerAccident`.
    ...
    **Example Usage**
    ...
    This example Rule will verify if the policy has property damage coverage and if the limitPerAccident on the coverage is greater than or equal to $50,000.
