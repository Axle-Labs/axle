...
```
  ...
  Evaluates whether input property matches the property that is listed on the Axle policy for `policy.type=auto | home | condo`

  ### Policy Type: `auto` and Year-Make-Model

  Evaluates if provided year-make-model matches year-make-model listed as one of the properties on the `policy.properties.type=vehicle`

  | Status    | Cause                                                                |
  | --------- | -------------------------------------------------------------------- |
  | `pass`    | Exact match on the year and high similarity threshold score on `make` and `model`  |
  ...
  | `unknown` |  Axle policy `properties` array is empty                              |
  ...
  **Example Usage**
  ...
  ```typescript Example Rule Response
    ...
    ### Policy Type: `auto` and VIN

    Evaluates if provided VIN matches the VIN listed as one of the properties on the `policy.properties.type=vehicle`

    | Status    | Cause                                                                |
    | --------- | -------------------------------------------------------------------- |
    | `pass`    | VIN matches one of the properties  |
    ...
    **Example Usage**
    ...
    ### Policy Type: `home | condo` and Address

    For `policy.type = home | condo` you can provide `address` as an input to validate against all the properties listed on the Axle policy. 
