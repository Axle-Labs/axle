...
<Info>
  You can specify fields when generating an Ignition token to handle Ignition status updates (see [Ignition Status Handling](/api-reference/ignition/ignition-status-handling) for more details) as well as attach user information or other metadata (see [Start Ignition](/api-reference/ignition/start-ignition) for more details). 
</Info>
