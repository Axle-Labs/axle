...
authorized access to a selected policy, OR if `acceptManual` is enabled, when the user
...
policy, OR if `acceptManual` is enabled, when the user opts out of sharing the
...
carrier, OR if `acceptManual` is enabled, when Axle is unable to collect policy
