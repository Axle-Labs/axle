...
In order to prevent the Axle system from becoming overwhelmed with requests, we have implemented rate limiting on our API endpoints. The thresholds are high enough that we do not expect any legitimate use case to be affected, but if you are experiencing issues, please reach out to us.

If rate limiting does occur, instead of the expected response, you will receive a `429 Too Many Requests` response code.
