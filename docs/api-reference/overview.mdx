...
- x-destination-client-id — unique identifier for the destination client (optional field used by platform integrations on select routes)
...
<Tip>
  *Optional* fields, such as `Policy.coverages["BI"].property` or
  ...
  - `null` - The carrier data source supports the field, but no information is present
  for the selected Account or Policy. 
  - `undefined` - The carrier data source supports the field and data is available that specifies that the field does
  not exist on the Account or Policy.
</Tip>
