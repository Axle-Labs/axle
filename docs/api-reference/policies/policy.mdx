...
List of entities (such as an individuals or businesses) afforded direct coverage by the policy.
    ...
    <Expandable title="Insured">
      ...
      <Tip>
        ...
        If your application requires matching your users against policy insureds, we recommend combining `firstName` and `lastName` into a single value and using a "fuzzy" matching algorithm with a strict threshold.
      </Tip>
