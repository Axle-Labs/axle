...
List of entities (such as an individuals or businesses) afforded direct coverage by the policy.
    ...
    <Expandable title="Insured">
      ...
      <Tip>
        ...
        If your application requires matching your users against policy insureds, we advise combining `firstName` and `lastName` into a single value and then using "fuzzy" matching algorithms with strict thresholds.
      </Tip>
...
List of documents (such as declaration pages and policy agreements) associated with the policy.
    ...
    <Expandable title="Document">
        ...
        <ResponseField name="type" type="array[string]" >
          The type of information contained in the document, as specified by carrier or user. A document may have multiple types, but each policy will have only 1 document of each type.
