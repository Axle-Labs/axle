...
<ParamField body="effectiveDate" type="string">
  ...
  by the insurance carrier. The effective date may be in the future for policies that renew early.
</ParamField>
...
Rules applied:
1. Excluded drivers are not included for `auto` insurance policies.
2. Entities listed as Additional Insureds are not included within the list of Insureds but are included under the list of Third Parties as an "interest".
  ...
  <Expandable title="Insured">
    ...
    <ResponseField name="type" type="string">
      The role of the insured individual, indicating whether they are the primary policyholder / user or a secondary insured party. Note that this field is in beta and is subject to modification.
...
List of external parties with interest in the policy.
    ...
    <Expandable title="ThirdParty">
        ...
        <ResponseField name="type" type="string" default="interest">
          The role of the Third Party associated with the insured property. Note that this field is in beta and is subject to modification.
...
Rules applied:
1. There will only be at most one document of each type listed in the documents array.
