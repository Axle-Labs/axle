...
<ParamField body="type" type="string">
  ...
  Rules applied:
  1. `auto` does not include commercial or business auto insurance policies.
  2. `home` includes HO-1, HO-2, HO-3, HO-5, HO-7, and HO-8 homeowners insurance policies. It does not include HO-4 (i.e. renter’s policies).
  3.  `condo` is equivalent to an HO-6 insurance policy.
  4. `other` encompasses any policy type that does not fall into one of the four described above. An example would be a renter’s policy.
</ParamField>
...
<ParamField body="isActive" type="boolean (optional)">
  ...
  1. If the policy status defined by the insurance carrier is an enum value, the Axle Service conservatively maps only "active" and active-like enum values to isActive `true` and the remainder as `false`.
  2. Pre-effective policies are policies that have been purchased but do not go into effect until a future date. The following applies to these policies:
  - Pre-effective policies of type `home` or `condo` will be marked as isActive `true` if policy data is able to be retrieved.
  - Pre-effective policies of type `auto` will be marked as isActive `false`.
</ParamField>
...
List of coverage types and levels offered by the policy.
    ...
    <Expandable title="Coverage">
        ...
        <ResponseField name="code" type="string" required>
          Available options for an `auto` insurance policy: 

          `BI` `PD` `UMBI` `UMPD` `UIMBI` `UIMPD` `UUIMBI` `UUIMPD` `COLL` `COMP`
        ...
        <ResponseField name="label" type="string" required>
          A human-readable name describing the type of insurance coverage provided by the policy.
        </ResponseField>
...
Rules applied:
1. Excluded drivers are not included.
2. Entities listed as Additional Insureds are not included within the list of Insureds but are included under the list of Third Parties as an "interest".
  ...
  <Expandable title="Insured">
    ...
    <ResponseField name="type" type="string">
      The role of the insured individual, indicating whether they are the primary policyholder or a secondary insured party.

      Available options:

      `primary` - The main policyholder, the individual who holds and is primarily responsible for the insurance policy.
    ...
    <ResponseField name="dateOfBirthYear" type="string">
      The birth year of the insured individual.
    </ResponseField>

    <ResponseField name="licenseNo" type="string">
      The driver's license number of the insured individual.
    </ResponseField>

    <ResponseField name="licenseState" type="string">
      The state that issued the insured individual's driver's license.
    </ResponseField>

    <ResponseField name="licenseStatus" type="string">
      The current status of the insured individual's driver's license.
    </ResponseField>

    <ResponseField name="dateOfBirth" type="string">
      The insured individual's date of birth in a standard date format.
    </ResponseField>
...
List of external parties with interest in the policy.
    ...
    <Expandable title="ThirdParty">
        ...
        <ResponseField name="type" type="string" default="interest">
          ...
          `lienholder` - A financial institution or individual that holds a legal claim on the insured property. 
          - This includes mortgagees.

          `lessor` - The owner or leasing company that leases the insured property to the policyholder under a lease agreement. 
          - This includes loss payees if the third party is also assessed as an Additional Insured.

          `interest` - Any third party with a financial interest in the insured property. 
          - This option serves as the conservative default used in the following cases:
            1. The third party is specifically listed as an "additional interest"
            2. The exact carrier-provided third party enum is not currently supported in the Axle Schema (e.g. loss payee, additional insured, creditor, etc.)
            3. The third party falls across options (e.g. a third party that is both a loss payee and a lienholder)
            4. The specific role of the third party is uncertain or not clearly defined.
        </ResponseField>
...
List of documents (such as declaration pages and policy agreements) associated with the policy.
    ...
    <Expandable title="Document">
        ...
        <ResponseField name="type" type="array[string]" >
          ...
          `declaration-page` - A summary document that outlines the key details of the insurance policy, including coverage limits, premiums, and insured parties. 
            - This includes renewal policy documents and amended policy documents.


          `policy-agreement` - The formal contract between the insurer and the policyholder that specifies the terms and conditions of the insurance coverage.
            - This includes insurance policy contract documents.
