...
2. isActive `null` indicates that the insurance carrier did not specify if the policy is active.
  ...
  <Info>
    ...
    If a `policy.modified` [event](/guides/policy-events) indicates that a policy’s `isActive` field has changed to `null`, then it is
...
2. Entities listed as Additional Insureds are not included within the list of Insureds but are included under the list of Third Parties as an "interest".
  ...
  <Expandable title="Insured">
    ...
    <ResponseField name="licenseStatus" type="string or null">
      The current status of the insured individual's driver's license. **Note:** This field is only available for select carriers.
    </ResponseField>
