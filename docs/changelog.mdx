...
- Add `null` type support for individual fields in Account and Policy response objects to improve data transparency and actionability
  - _Non-optional_ fields will return `null` values when:
  ...
  - _Optional_ fields, such as `Policy.coverages["BI"].property` or `Policy.coverages["UMPD"].deductible` may return `null` or `undefined`
...
- Clarification
  - `Policy.insureds.type` is not a required field, and therefore should be used for display purposes only. The Axle team is working on refining this field to add more specificity and accuracy.
